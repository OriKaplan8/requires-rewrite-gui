...
class MongoData:
    ...
    def __init__(self, root, connection_string, annotators):
        """
    ...
    def load_files(self):
        ...
        for annotator in self.annotators:
            username = annotator[0]
            filename = annotator[1]
        ...
        filled_files = {}
        for name, jdata in files.items():
            filled_files[name] = self.fill_dialogs(jdata)
        print(filled_files)
        self.check_needs_clarification(list(filled_files.values())[0])
        return filled_files
